...
Date: 2026-09-10. **Railway deployment, PostgreSQL connectivity and public HTTPS health: PASS.** Remote API/export checks have been verified across the recorded attempts and independent package rechecks. Final mobile-layout correction, redeployment and browser acceptance remain in progress.
...
| Web deployment | `e8f6f08e-5921-4139-bd07-cb3e1d4c4cfd` — SUCCESS |
...
| Web image digest | `sha256:83d85934265f9e3c19d0d0b4ee20e596b2534beeaa4fdc3f426076b960ad3e4e` |
...
The service uses `/health/`, a 120-second readiness timeout, `ON_FAILURE` restart policy with three retries and application sleeping disabled. The health endpoint checks database access and is exempt from HTTPS redirection for Railway's readiness probe. Public application traffic uses HTTPS with the configured proxy/security settings.
...
| PostgreSQL/container CI | PASS: [Actions run 34441801870](https://github.com/haivpm2-web/aqua_iceeis26/actions/runs/34441801870) executed all 82 tests on PostgreSQL 17, migrations, production checks and an actual Linux Docker/Gunicorn health check |
...
| Remote browser and downloads | Routes/charts/CSV/PNG/PDF/ZIP passed; mobile overflow requires correction and final rerun |
...
`evidence/final-remote-api-verification.json` records aggregate PASS for all required API/export checks on runs 1 and 3 across attempts and independent package rechecks. Neither original driver report is relabelled PASS. Remote browser routes, charts and CSV/PNG/PDF/ZIP downloads passed. The longer remote run name exposed horizontal overflow on mobile; the layout correction, redeployment and final browser rerun are pending. Client request failures are recorded as observed; their root cause has not been established.

Evidence files: `evidence/final-ci-verification.json`, `evidence/final-railway-web-deployment.json`, `evidence/final-railway-postgres-deployment.json`, `evidence/final-railway-startup.json` and `evidence/final-remote-health.json`. The first public status snapshot precedes demo ingestion and therefore correctly reports zero samples. Later acceptance records describe the dedicated SIMULATED runs.
